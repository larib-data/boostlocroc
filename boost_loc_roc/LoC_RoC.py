...
def base_plot_spectrogram(time_loc, time_roc, signal, Fs, time, t_proba=None, proba=None, params_LoC=None, params_RoC=None, patient_id=None, debug=False):
    """Base function to plot spectrogram with optional probability and debug plots."""
...
def plot_spectrogram(time_loc, time_roc, signal, Fs, time, patient_id=None):
    ...
    base_plot_spectrogram(time_loc, time_roc, signal, Fs, time, t_proba=None, proba=None, patient_id=patient_id)

def plot_spectrogram_proba(time_loc, time_roc, signal, Fs, time, t_proba, proba, patient_id=None):
    """Plot the spectrogram with probabilities."""
    base_plot_spectrogram(time_loc, time_roc, signal, Fs, time, t_proba, proba, params_LoC=None, params_RoC=None, patient_id=patient_id)
    
def plot_spectrogram_debug(time_loc, time_roc, signal, Fs, time, t_proba, proba, params_LoC, params_RoC, patient_id=None):
    """DEBUG TOOL: Similar to plot_spectrogram with additional sigmoids visible for inspection."""
    base_plot_spectrogram(time_loc, time_roc, signal, Fs, time, t_proba, proba, params_LoC=params_LoC, params_RoC=params_RoC, patient_id=patient_id, debug=True)
