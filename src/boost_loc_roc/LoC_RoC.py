...
def base_plot_spectrogram(
    ...
    pxx, freqs, bins, im = ax[0].specgram(
        signal, Fs=Fs, NFFT=int(nperseg), mode="psd", cmap="jet", noverlap=int(noverlap)
    )
...
def Truncate_fif(raw, electrode=1):
    ...
    data = data[:, mask2]
    ch_types = [
        mne.io.pick.channel_type(raw.info, idx) for idx in range(raw.info["nchan"])
    ]

    info = mne.create_info(
        ch_names=raw.info["ch_names"], sfreq=raw.info["sfreq"], ch_types=ch_types
    )
