...
def evaluate(
    ...
    if display_pred:
        if parquet is None or raw is None:
            raise ValueError("parquet and raw must be passed with display_pred")

        if zoom:
            plot_zoom(parquet, raw, y_true, y_pred, precision, subject_name)
            plt.show()
        else:
            plot_pred(
                parquet,
...
def evaluate_model(
    ...
    for idx in tqdm(X.index.unique()):
        ...
        if display_sup_60 and abs(delta_loc) > 60:
            plot_zoom(parquet, raw, y_true, y_pred, precision, idx, is_loc=True)
            plt.show()

        if display_sup_60 and abs(delta_roc) > 60:
            plot_zoom(parquet, raw, y_true, y_pred, precision, idx, is_loc=False)
            plt.show()
    ...
    return (scores_loc, scores_roc, deltas_loc, deltas_roc, y_true_list), y_pred_list
